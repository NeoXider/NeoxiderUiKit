...
Фон задаётся **на каждую страницу отдельно** (`UiKitConfig.PageEntry.backgroundSprite`), а не глобальным слоем: у меню свой фон, у игрового экрана его нет (там за прозрачным UI рисуется игровой мир). При бинде страница вешает спрайт на свой `ScreenRoot` (`background-image`, `ScaleAndCrop`); страница без спрайта остаётся прозрачной.

При генерации `UiKitConfigUpdater` расставляет фоны сам (add-only):
- страница со **screen-level** элементом `background` в UXML (например `loading`) рендерит свой фон из вёрстки — конфиг не трогается;
- прозрачная **не-игровая** страница (`mainmenu` и подобные) получает «сценический» фон дизайна — самую частую screen-level фоновую картинку (затемнения `background` внутри попапов игнорируются);
- **игровые** страницы (id начинается с `game`) остаются прозрачными.

Поскольку `loading` и `mainmenu` используют один и тот же фон, переход между ними — бесшовный dissolve: входящая страница появляется мгновенно (её непрозрачный фон уже на месте), уходящая растворяется поверх (`showPreset = none`, `hidePreset = fade`, `BackwardOnly`) — без зазора и вспышки. Любой фон правится per-страница в окне/конфиге.

## Кнопки: пресеты нажатия

`ButtonView` вешает на каждую кнопку анимацию нажатия через класс `uikit-pressed` (USS-переход). Пресет выбирается глобально в `UiKitConfig.buttonPressPreset`:

| Пресет | Эффект |
|---|---|
| `scale` (по умолчанию) | сжатие до 0.92 |
| `sink` | лёгкое проседание вниз + 0.97 |
| `pop` | увеличение до 1.08 |
| `none` | без анимации |

Точечное переопределение — `ButtonView.SetPressAnimation(IUiAnimator)` или свой класс в override-USS по образцу `.fui_type_button.uikit-press-<имя>.uikit-pressed`.

## Аудио-тумблеры

Кит из коробки биндит элементы с именами `toggle_sound`/`button_sound` и `toggle_music`/`button_music` к `UiKit.Audio` (двусторонне, с классами `uikit-toggle`/`is-off`). Если дизайнер не разметил тумблеры именами (как в MY_GAME_DESIGN — там строки «трек + ползунок» из image-элементов), обвязка пишется в user-partial страницы: см. `Assets/UiKit/Views/AudioToggleWiring.cs` — клик по треку/ползунку переключает `SoundOn/MusicOn`, текстуры состояний снимаются с исходного арта двух строк (зелёная=вкл, красная=выкл), ползунок скользит USS-переходом, при каждом открытии попапа состояние перечитывается из `UiKit.Audio` — настройки и пауза всегда показывают одно и то же.
