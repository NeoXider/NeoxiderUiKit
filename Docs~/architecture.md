...
## UiBackgroundLayer — постоянный фон

`Neo.UIKit.UiBackgroundLayer` — всегда включённый фон позади игрового мира и UI. Реализован обычным `Canvas` в режиме **Screen Space – Camera** (`worldCamera = Camera.main`, `planeDistance` у дальней плоскости) с одним растянутым `Image` и `sortingOrder -1000` — мировые объекты игры рисуются поверх него, UI Toolkit-панели поверх всего.

- Спрайт берётся из `UiKitConfig.backgroundSprite`; при генерации выбирается автоматически — фоновая картинка **уровня экрана** дизайна (элементы `background` внутри попапов игнорируются: это полупрозрачные затемнения-подложки, а не фон).
- `SceneObjectBuilder` создаёт объект `UI_Background` при сборке сцены, когда спрайт задан в конфиге; порядок меняется полем `backgroundSortingOrder`.
- Прямой UI Toolkit-путь (PanelRenderer с отрицательным sorting) намеренно не используется: overlay-панель не может оказаться позади игрового мира.
